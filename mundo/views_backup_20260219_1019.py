...
def home(request):
    ...
    try:
        ...
        if lat != 0.0 and lon != 0.0:
            ...
            # === DATOS ESPECÍFICOS PARA VENTANAS LATERALES ===
            
            # PRÓXIMAS HORAS COMPACTAS (Panel Izquierdo)
            hora_actual = hora_local_dt.hour
            proximas_horas = []
            for h in tira_hoy:
                if h['tipo'] == 'normal':
                    hora_item = int(h['hora'].split(':')[0])
                    if hora_item >= hora_actual and len(proximas_horas) < 8:  # Próximas 8 horas
            ...
            contexto['proximas_horas_compactas'] = proximas_horas
            
            # PRÓXIMOS DÍAS COMPACTOS (Panel Derecho)
            proximos_dias_compactos = []
            for i, dia in enumerate(lista_pronostico[:5]):  # Solo 5 días para panel lateral
                proximos_dias_compactos.append({
                    'nombre': dia['nombre_dia'][:3] if dia['nombre_dia'] != 'HOY' else 'HOY',  # Abreviar
                    'fecha': dia['fecha_corta'],
                    'icono': dia['icono'],
                    'max': round(dia['max']),
                    'min': round(dia['min']),
                    'desc': dia['desc'][:15] + '...' if len(dia['desc']) > 15 else dia['desc'] # Descripción corta
                })
            contexto['proximos_dias_compactos'] = proximos_dias_compactos
            ...
            if uv > 8:
                alertas_activas.append({
                    'tipo': 'uv_extremo',
                    'mensaje': f'Índice UV extremo: {uv}',
                    'descripcion': 'Evita exposición solar. Usa protector SPF 50+',
                    'color': '#ef4444',
                    'icono': 'sun',
                    'severidad': 'alta'
                })
                ...
                alertas_activas.append({
                    'tipo': 'uv_alto', 
                    'mensaje': f'Índice UV alto: {uv}',
                    'descripcion': 'Usa protector solar y gafas',
                    'color': '#f59e0b',
                    'icono': 'sun',
                    'severidad': 'media'
                })
            ...
            if actual['wind_speed_10m'] > 50:
                alertas_activas.append({
                    'tipo': 'viento_fuerte',
                    'mensaje': f'Vientos fuertes: {round(actual["wind_speed_10m"])} km/h',
                    'descripcion': 'Evita actividades al aire libre. Peligro de objetos volando',
                    'color': '#ef4444',
                    'icono': 'wind',
                    'severidad': 'alta'
                })
            elif actual['wind_speed_10m'] > 25:
                alertas_activas.append({
                    'tipo': 'viento_moderado',
                    'mensaje': f'Vientos moderados: {round(actual["wind_speed_10m"])} km/h',
                    'descripcion': 'Precaución con paraguas y objetos ligeros',
                    'color': '#f59e0b',
                    'icono': 'wind',
                    'severidad': 'media'
                })
            ...
            if actual['temperature_2m'] > 35:
                alertas_activas.append({
                    'tipo': 'calor_extremo',
                    'mensaje': f'Calor extremo: {round(actual["temperature_2m"])}°C',
                    'descripcion': 'Mantente hidratado. Evita ejercicio intenso',
                    'color': '#ef4444',
                    'icono': 'thermometer',
                    'severidad': 'alta'
                })
            elif actual['temperature_2m'] < 0:
                alertas_activas.append({
                    'tipo': 'frio_extremo',
                    'mensaje': f'Frío extremo: {round(actual["temperature_2m"])}°C',
                    'descripcion': 'Riesgo de engelamiento. Abrígate bien',
                    'color': '#38bdf8',
                    'icono': 'snowflake',
                    'severidad': 'alta'
                })
            
            # Alerta Visibilidad Reducida
            if vis_metros < 500:
                alertas_activas.append({
                    'tipo': 'niebla_densa',
                    'mensaje': f'Visibilidad crítica: {vis_km} km',
                    'descripcion': 'Conduce con extrema precaución. Usa luces',
                    'color': '#6b7280',
                    'icono': 'fog',
                    'severidad': 'alta'
                })
                ...
                alertas_activas.append({
                    'tipo': 'niebla_moderada',
                    'mensaje': f'Visibilidad reducida: {vis_km} km',
                    'descripcion': 'Precaución al conducir',
                    'color': '#f59e0b',
                    'icono': 'fog',
                    'severidad': 'media'
                })
            ...
            lluvia_siguiente_4h = sum([datos_por_dia[fecha_hoy][i]['lluvia'] for i in range(min(4, len(datos_por_dia[fecha_hoy]))) if datos_por_dia[fecha_hoy][i]['tipo'] == 'normal']) / 4 if fecha_hoy in datos_por_dia else 0
            if lluvia_siguiente_4h > 80:
                alertas_activas.append({
                    'tipo': 'lluvia_intensa',
                    'mensaje': f'Lluvia intensa esperada: {round(lluvia_siguiente_4h)}%',
                    'descripcion': 'Lleva paraguas. Posibles inundaciones',
                    'color': '#3b82f6',
                    'icono': 'rain',
                    'severidad': 'media'
                })
            ...
            if code >= 95:
                alertas_activas.append({
                    'tipo': 'tormenta_electrica',
                    'mensaje': 'Tormenta eléctrica activa',
                    'descripcion': 'Busca refugio inmediatamente. Evita espacios abiertos',
                    'color': '#7c3aed',
                    'icono': 'thunderstorm',
                    'severidad': 'crítica'
                })
            
            contexto['alertas_activas'] = alertas_activas
            contexto['total_alertas'] = len(alertas_activas)
